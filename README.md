...
### ✅ Решение
Очень горжусь тем, что смог сам решить задачу уровня middle, так ещё и хорошим способом🥳!
```python
class Solution:
...
что округление делалось не так как он задумывал🤨. Спустя время я разобрался, что дело в том как округляет python отрицательные числа, 
округление идет в сторону `-∞`, а нам надо к нулю, int срабатывает потому-что просто, отбрасывает дробную часть🫡.


Дальше, как обычно, бывает в задачах по стэка, мы создаем стэк `operand_list` и итерируем по входному списку, 
