...
* `left` - идет сначала массива к его конец
* `right` - идет с конца в начало

Накаждом шаге сравниваем символы на которые ссылаются указатели и так идем до середины строки, если по пути символы вдруг не совпали, то выдаем `Fasle`, если прошли и всё окей, то возвращаем `True`.
