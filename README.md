...
Тут решение находиться довольно быстро, если смотреть на задачу не с точки перемещения нулей в конец массива,
...
* `slow` - указывает на место куда нужно вставить следуеще не нулевое число. Изначально находиться в начале массива.
...
В таком случаем, мы просто меняем местами число на которое указывает `fast` с числом на которое указывает `slow` и сдвигаем `slow` на 1.
...
На этом шаге `fast` нашел не нулевое число.
...
Опять ненулевое число от fast и последнее в массиве. Делаем всё те же действия, что и на предидущем шаге.
