...
Мы будем итерировать с конца `nums2` с указателем `p2` и с конца значимых элементов `nums1` с указателем p1 (нули из условия задачи мы не учитываем, по условию задачи).
...
| p2   |       |       |       | 🔽    |       |       |
| num2 |       |       |       | `2  ` | ` ✅ ` | `✅  ` |
На третьем шаге мы сравниваем мксимальный элемент последний (_максимальный_) элемент `nums1` и 
последний (_максимальный_) элемент `nums2`, и тут оказывается что `2 < 4`. В таком случае мы двигаем `4` вправо и сравниваем `2` со следующим элементом.
